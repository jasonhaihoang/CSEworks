...
Name:	Jason Hoang
Course:	CSCE 3110
Assignment:	PROGRAM 3 - Binary Search Trees - PART C
Due Date:	7-17-2015
****************/

#include <iostream>
#include <stdlib.h>	//for use of rand()
#include <time.h>	//For seeding rand() and for clock()
#include <iomanip>	//for use of setprecision()

int AlternateIndex;	//global variable to keep track of Alternating between Left Tree and Right Tree Recursion
using namespace std;
...
class BinaryTree	//Probably unecessary class to hold tree functions
{
	public:
		...
		int SmallestRightTree(node*& node);	//Successor function
};
...
int main()
{
	...
	double t;			//CPU time holder
	
	srand(time(NULL));			//seed random function
	AlternateIndex = rand() % 100 + 1;	//set up the Alternate Index
	cout << "\nAlternateIndex = " << AlternateIndex << endl;
	...
	cout << endl;
	ExampleA->insert(8, root);	//HARD CODED node inserts
	...
	c1 = clock(); // Stop clock
	t = (c0-c1)/ (double) CLOCKS_PER_SEC;		//Calculate CPU time
...
void BinaryTree::remove(int value, node*& leaf)
{
	...
	{
		if((AlternateIndex%2) != 0)	//odd index
		{
		...
		{
			leaf->value = SmallestRightTree(leaf->right);	 //retrieve the successor value to replace the deleted node
			cout << "REPLACEMENT -- " << leaf->value << endl;
			remove(leaf->value, leaf->right);                //remove the successor node from original place
		}
	}
	else 	//if (node-key == key) and 1 child or no children - DELETE
	{
		...
		else if(leaf->right != NULL)  //1 right child
                {
		...
		else			//FINAL DELETION -- end of recursive deletion
		{
			...
			AlternateIndex = rand() % 100 + 1;	//reset the AlternateIndex to a new number for the next iteration of DELETE
			return;
		}
        }
}
...
int BinaryTree::LargestLeftTree(node*& leaf)
{
	...
	value = temp->value;		//predecessor found - replacement
	return value;			//return the value of the replacement node
}

//SUCCESSOR Function
int BinaryTree::SmallestRightTree(node*& leaf)
{
        node *temp;
        int value;
        temp = leaf;	//temporary node to find the Successor
	
        while(temp->left != NULL)
        {
                temp = temp->left;	//traverse subtree until predecessor is found
	}
	value = temp->value;		//successor found - replacement
	return value;			//return the value of the replacement node
}
