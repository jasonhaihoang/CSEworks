...
class Process{			//This class serves as the data structure containing the process 3-tuple
	private:
		...
		bool memAllocated; // has MEM been allocated?
		int SIZE_OF_MEM_BLOCK; // 20 MB is 20480 KB
	...
	public:
		...
		bool isRAMAllocated();
		// my_malloc
		int my_malloc(int numOfBlocksToMalloc);
		// my_free
		int my_free(int offset, int numOfBlocksToFree);
		// for debugging only
		void printMEM(unsigned int x);
};
...
Process::Process(){
	...
	totalWait2 = 0;
	offset = 0;
	memAllocated = false; // has MEM been allocated?
	SIZE_OF_MEM_BLOCK = 20480; // 20 MB is 20480 KB
	RAM = (int *) malloc(SIZE_OF_MEM_BLOCK * sizeof(int));
	if (RAM != NULL)
	{
		for (int i = 0; i < SIZE_OF_MEM_BLOCK; i++) // loop through all RAM offsets
		...
		for(i=0; i<numProc; i++)
		{
			...
			else if((plist[i].run == false) && (plist[i].procDone1 == false))
			{
				if(nextProc == 0)
				{
					...
					if(quantum == 0)
					{
						//plist[i].offset = malloc((plist[i].memSize)*sizeof(int));
...
int Process::my_malloc(int numOfBlocksToMalloc)
{
	...
	for (i = 0, tempNum = 0; i < SIZE_OF_MEM_BLOCK; i++) // loop through all RAM offsets
	{
		...
		{
			for (int j=0; j < numOfBlocksToMalloc; j++, i--) // loop through block size
...
int Process::my_free(int offset, int numOfBlocksToFree)
{
	...
	for (int i = 0; i < numOfBlocksToFree; i++)
...
void Process::printMEM(unsigned int x)
{
	for (int i = 0; i<x && i<SIZE_OF_MEM_BLOCK; i++)
		cout << RAM[i];
	cout << endl;
};
...
int main() {
	...
	if (!process.isRAMAllocated()) // call malloc
	{
		return 0;
	}
	...
	cout << "The the memory size for each process has been generated for each process.\n" << endl;
